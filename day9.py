...
Maybe you can report any environmental instabilities you find to someone so the oasis can be
...
Once all the values in your latest sequence are zeroes, you can extrapolate what
...
def extrapolate_value(
    number_sequence: Sequence[int], extrapolate_backward: bool = False
) -> int:
    if all(number == 0 for number in number_sequence):
        return 0
    else:
        extrapolated_value = extrapolate_value(
            [
                b - a if extrapolate_backward else a - b
                for a, b in zip(number_sequence[1:], number_sequence)
            ],
            extrapolate_backward,
        )
        return number_sequence[0 if extrapolate_backward else -1] + extrapolated_value


def sum_extrapolated_values(filename: str, extrapolate_backward: bool = False) -> int:
    with open(filename) as f:
        file_content = f.read().strip()
        number_sequences = list(map(extract_numbers, file_content.splitlines()))
        return sum(
            map(lambda x: extrapolate_value(x, extrapolate_backward), number_sequences)
        )


def part1(filename: str) -> int:
    return sum_extrapolated_values(filename)
...
# from bottom to top eventually reveals the new left-most history value: 5.
#
# Doing this for the remaining example data above results in previous values of -3 for the first history and
# 0 for the second history. Adding all three new values together produces 2.
#
...
def part2(filename: str) -> int:
    return sum_extrapolated_values(filename, extrapolate_backward=True)
