...
contains a list of left/right instructions, and the rest of the documents seem to describe some kind of network of
labeled nodes.
...
def parse_input(data: str):
    ...
    assert not len(lines[1].strip())

    pattern = re.compile(r"(\w{,3}) = \((\w{,3}), (\w{,3})\)")
    for line in lines[2:]:
        match = pattern.fullmatch(line)
        node = match.group(1)
        graph[node]["L"] = match.group(2)
        graph[node]["R"] = match.group(3)

    return directions, graph


def solve(directions, graph, start, termination=lambda current, dst: current == dst):
    num_steps = 0
    current, dst = start, "ZZZ"
    for direction in cycle(directions):
        if termination(current, dst):
            return num_steps
...
def part1(filename: str):
    with open(filename) as f:
        directions, graph = parse_input(f.read())
        return solve(directions, graph, "AAA")
...
# all the paths at the same time until they all simultaneously end up at nodes that end with Z.
#
# For example:
#
# LR
...
# As you follow each left/right instruction, use that instruction to simultaneously navigate away
# from both nodes you're currently on. Repeat this process until all the nodes you're currently on end with Z.
...
def part2(filename: str):
    with open(filename) as f:
        ...
        return math.lcm(
            *(
                solve(
                    directions, graph, start, lambda current, dst: current.endswith("Z")
                )
                for start in filter(lambda node: node.endswith("A"), graph.keys())
            )
        )


day8 = AOCChallenge(8, part1, part2)
__all__ = [day8]
